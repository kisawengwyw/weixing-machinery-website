...
const cases = [
  ...
  ,['duplicate-attribute', s => s.replace('<main id="main-content" tabindex="-1">', '<main id="main-content" tabindex="-1"><svg focusable="false" focusable="false"></svg>')]
];
...
assert(checkHtml(valid().replace(' tabindex="-1"', '')).some(error => error.code === 'skip-target-focusable'), 'main without tabindex should report skip-target-focusable');
assert(checkHtml(valid().replace('tabindex="-1"', 'tabindex="0"')).some(error => error.code === 'skip-target-focusable'), 'main tabindex=0 should report skip-target-focusable');
const positiveMain = checkHtml(valid().replace('tabindex="-1"', 'tabindex="1"')).map(error => error.code);
assert(positiveMain.includes('skip-target-focusable') && positiveMain.includes('positive-tabindex'), 'main tabindex=1 should report focusability and positive tabindex errors');
assert(checkHtml(valid().replace('href="#main-content"', 'href="#other-content"').replace('<main ', '<div id="other-content"></div><main ')).some(error => error.code === 'skip-target-main'), 'skip link to another existing target should report skip-target-main');
console.log(`Accessibility checker fixtures passed: ${cases.length + 6} failures and 5 valid structure/page cases.`);
