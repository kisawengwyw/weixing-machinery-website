...
export function checkHtml(html, file = 'fixture.html') {
  ...
  const is404 = /(?:^|\/)404\.html$/i.test(file);
  const ids = new Map();
  ...
  for (const match of html.matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a>/gi)) {
    ...
    const active = /\bactive\b/.test(attr(tag, 'class') || '');
    if (active && attr(tag, 'aria-current') !== 'page' && !is404) add('current-link', 'active link needs aria-current="page"');
    if (is404 && attr(tag, 'aria-current') === 'page' && href && !href.startsWith('#')) add('current-page-target', '404 links to another page cannot be aria-current="page"');
  }
  ...
  if (!skip) add('skip-link', 'page needs a skip link');
    ...
    const skipHref = attr(skip, 'href') || '';
    if (!skipHref.startsWith('#')) add('skip-target-page', 'skip link must target the current document');
    const target = skipHref.startsWith('#') ? skipHref.slice(1) : '';
    if (!target || !ids.has(target)) add('skip-target', 'skip link target does not exist');
    if (skipHref !== '#main-content') add('skip-target-main', 'skip link must target #main-content');
    const mainTarget = tags(html, 'main').find(tag => attr(tag, 'id') === 'main-content');
    if (!mainTarget || attr(mainTarget, 'tabindex') !== '-1') add('skip-target-focusable', 'main#main-content must use tabindex="-1"');
  }
