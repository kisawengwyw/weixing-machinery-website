...
| Accessibility errors before | 944 |
| Accessibility errors after | 0 |

“Before” is the new checker's deterministic result against the branch's `master` parent. “After” is the same checker against this change. The count is not a browser accessibility score.
...
All checked images retain their existing alt text, dimensions, responsive sources, loading priorities, and gallery order. Every page now has one `main#main-content` target and a localized skip link. RFQ upload controls have explicit labels and `aria-describedby` help associations; existing required attributes and polite live status regions remain unchanged. Table header cells now use `scope="col"` or `scope="row"` according to their existing structure, without changing technical data.
