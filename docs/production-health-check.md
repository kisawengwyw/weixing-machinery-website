...
Open **Actions**, choose **Check Production Website**, and select **Run workflow** for an on-demand production audit. The workflow also runs every Monday at 02:20 UTC and after relevant website-file pushes to `master`. Pull requests run only the local fixture test through the existing site-check workflow; they do not contact production.
